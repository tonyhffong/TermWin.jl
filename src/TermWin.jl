...
function initsession()
    ...
    if nc_context === nothing
        ...
        nc_context = NC.NotcursesObject(opts=NC.Options(
            flags = UInt(NC.LibNotcurses.NCOPTION_SUPPRESS_BANNERS)
        ))
...
function winnewcenter( ysize, xsize, locy=0.5, locx=0.5 )
    ...
    plane = NC.Plane(NC.LibNotcurses.ncplane_create(rootplane.ptr,
